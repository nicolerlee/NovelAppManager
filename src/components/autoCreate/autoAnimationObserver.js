...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const checkInterval = setInterval(() => {
    ...
    const freeLayoutMain = document.querySelector('.free-layout-main');
    if (freeLayoutMain) {
      // 清除定时器
      clearInterval(checkInterval);
      
      // 检查是否进入自动创建模式
      const checkAutoCreateMode = () => {
        // 检查appStore对象是否存在
        if (window.appStore && window.appStore.getAutoTaskConfig) {
          const autoTaskConfig = window.appStore.getAutoTaskConfig;
          return autoTaskConfig.taskStatus === 'running' && autoTaskConfig.currentTask !== null;
        }
        return false;
      };
      ...
      const canAnimate = () => {
        return (
          ...
          document.querySelector('.phone-app-area')
        );
      };
      
      // 执行动画的函数
      const executeAnimation = () => {
        console.log('执行组件飞入动画...');
      ...
      const attemptAnimation = () => {
        if (canAnimate()) {
          ...
          // 等待300毫秒后重试
          setTimeout(attemptAnimation, 300);
        }
      };
